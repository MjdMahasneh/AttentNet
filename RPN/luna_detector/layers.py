...
class AttendHere_type1(nn.Module):

    def __init__(self, n_features):
        super(AttendHere_type1, self).__init__()
        self.conv = nn.Conv3d(n_features, n_features, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn = nn.BatchNorm3d(n_features)
...
class AttendHere_type2(nn.Module):

    def __init__(self, n_features):
        super(AttendHere_type2, self).__init__()
        self.conv = nn.Conv3d(n_features, 1, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn = nn.BatchNorm3d(1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        x_out = self.conv(x)
        print('x_out.size() in attention block', x_out.size())
        x_out = self.bn(x_out)
        scale = self.sigmoid(x_out)
...
class AttendHere_type3(nn.Module):
    def __init__(self, n_features):
        super(AttendHere_type3, self).__init__()

        self.maxpool3d = nn.MaxPool3d(kernel_size=3, stride=1, padding=1)
...
class AttendHere_type4(nn.Module):
    def __init__(self, n_features):
        super(AttendHere_type4, self).__init__()
...
class AttendHere_type5(nn.Module):

    def __init__(self, n_features):
        super(AttendHere_type5, self).__init__()

        self.adaptiveavgpool3d = nn.AdaptiveAvgPool3d(3)
        self.conv = nn.Conv3d(n_features, n_features, kernel_size=3, stride=1, bias=False)

        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        x_out = self.adaptiveavgpool3d(x)
...
class ResNeXtBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, cardinality, stride=1,
        ...
        self.tanhleakyrelu = TanhLeakyRelu()

        self.downsample = downsample
        self.stride = stride

        self.attendhere = AttendHere_type5(planes * self.expansion)
