...
class LungNodule3Ddetector(Dataset):
    ...
    def __init__(self, data_dir, split_path, config, phase='train', split_comber=None, cross_sect='',
                 randomize_on_wrap_around=False):
        assert (phase == 'train' or phase == 'val' or phase == 'test')
        assert (cross_sect == 'axial' or cross_sect == 'coronal' or cross_sect == 'sagittal')

        self.randomize_on_wrap_around = randomize_on_wrap_around
        ...
        for idx in idcs:
            ...
            if np.all(l == 0):
                ...
                elif cross_sect == 'coronal':
                    l_prime[:, 2] = l[:, 2]
                    l_prime[:, 1] = l[:, 0]
                    l_prime[:, 0] = l[:, 1]
                elif cross_sect == 'sagittal':
                    l_prime[:, 2] = l[:, 1]
                    l_prime[:, 1] = l[:, 0]
                    l_prime[:, 0] = l[:, 2]
    ...
    def __getitem__(self, idx, split=None):
        ...
        if self.phase != 'test':
            if idx >= len(self.bboxes):
                ...
                if self.randomize_on_wrap_around:
        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                imgs = imgs[0]
                if self.cross_sect == 'axial':
                    pass
                elif self.cross_sect == 'coronal':

                    imgs = np.swapaxes(imgs, 0, 1)
                elif self.cross_sect == 'sagittal':
